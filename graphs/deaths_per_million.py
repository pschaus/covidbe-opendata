...
def lines_deaths_per_million():
    ...
    MIN_DEATHS = 10
    filter_min_dead = data_pc.total_deaths < MIN_DEATHS
    data_pc = data_pc.drop(data_pc[filter_min_dead].index)
    
    # compute per habitant
    for i in countries:
        data_pc.loc[data_pc.location == i,"total_deaths"] = data_pc.loc[data_pc.location == i,"total_deaths"]/float(data_pop.loc[data_pop.country == i, "pop"])
    
        # get each country time series
    filter1 = data_pc["total_deaths"] > 1
    for i in countries:
    ...
    for i in range(0,len(countries)):
        ...
        data_countries_pc2[i]['log_deaths'] = np.log10(data_countries_pc2[i]["total_deaths"])
    ...
    base = alt.Chart(source, title = get_translation(
                en="COVID-19 Reported Deaths Per Million of Inhabitants",
    ...
    labels = pd.DataFrame([{'label': 'Doubles every 4 days', 'x_coord': 32, 'y_coord': 1.8},
    ...
    trend_4d = alt.Chart(source).mark_line(color="grey", strokeDash=[3,3]).encode(
        x = "n_days:Q",
        y = alt.Y("trend_4days:Q",  scale=alt.Scale(domain=(0, max(data_plot["log_deaths"])))),
    )
    
    trend_12d = alt.Chart(source).mark_line(color="grey", strokeDash=[3,3]).encode(
        x = "n_days:Q",
        y = alt.Y("trend_12days:Q",  scale=alt.Scale(domain=(0, max(data_plot["log_deaths"])))),
    )
