...
def display_hospitals():
    return [
        ...
        dbc.Row([
            dbc.Col(display_graphic(id='hospitalization-be-exponential-fit',
        ...
        dbc.Row([
            dbc.Col(display_graphic(id='hospitalization-be-smooth',
                              figure=hospi_smooth(),
                              config=dict(locale=str(get_locale())))),
        ]),
        html.H2(gettext("New daily hospitalizations Avg over 7 past days")),
        dbc.Row([
            dbc.Col(display_graphic(id='hospitalization-be-in-smooth',
                              figure=newin_smooth(),
                              config=dict(locale=str(get_locale())))),
        ]),
        html.H2(gettext("Total Deaths Avg over 7 past days")),
        dbc.Row([
            dbc.Col(display_graphic(id='death-covid-be-smooth',
                              figure=death_smooth(),
                              config=dict(locale=str(get_locale())))),
        ]),
