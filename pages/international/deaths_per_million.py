...
data = data.rename(columns={"Country/Region": "location", "variable": "date", "value": "total_deahs"})
...
MIN_DEATHS = 10
filter_min_dead = data_pc.total_deahs < MIN_DEATHS
data_pc = data_pc.drop(data_pc[filter_min_dead].index)

# compute per habitant
for i in countries:
    data_pc.loc[data_pc.location == i,"total_deahs"] = data_pc.loc[data_pc.location == i,"total_deahs"]/float(data_pop.loc[data_pop.country == i, "pop"])

    # get each country time series
filter1 = data_pc["total_deahs"] > 1
for i in countries:
...
for i in range(0,len(countries)):
    ...
    data_countries_pc2[i]['log_deahs'] = np.log10(data_countries_pc2[i]["total_deahs"])
...
base = alt.Chart(source, title = "COVID-19 Reported Deaths Per Million of Inhabitants").encode(
    x = alt.X('n_days:Q', title = "Days passed since reaching 1 reported death per million"),
    y = alt.Y("log_deahs:Q",title = "Log of reported deaths per million"),
...
trend_2d = alt.Chart(source).encode(
    x = "n_days:Q",
    y = alt.Y("trend_2days:Q",  scale=alt.Scale(domain=(0, max(data_plot["log_deahs"])))),
...
trend_4d = alt.Chart(source).mark_line(color="grey", strokeDash=[3,3]).encode(
    x = "n_days:Q",
    y = alt.Y("trend_4days:Q",  scale=alt.Scale(domain=(0, max(data_plot["log_deahs"])))),
)

trend_12d = alt.Chart(source).mark_line(color="grey", strokeDash=[3,3]).encode(
    x = "n_days:Q",
    y = alt.Y("trend_12days:Q",  scale=alt.Scale(domain=(0, max(data_plot["log_deahs"])))),
)
